...
def visualize_tables(hash_tables, plot_type="bar"):
    for table_idx, table in enumerate(hash_tables):
        keys = sorted(table.keys())
        keys_cat_counts = {}
        for idx, k in enumerate(keys):
            entries = table.get_val(k)
            cats = [e[1].split("_", maxsplits=1)[0] for e in entries]
...
def table_pca_scatter_plot(table_idx, keys, keys_cat_counts):
    all_categories = list(keys_cat_counts.keys())
    keys = np.array(keys)
    ...
    fig = go.Figure(
        data=go.Scatter(
            ...
            name=first_cat,
            mode="markers",
            marker=dict(size=first_cat_counts),
        )
    )
    for cat in all_categories[1:]:
        cat_counts = keys_cat_counts[cat]
        fig.add_trace(
            go.Scatter(
                x=key_components[:, 0] + 1.0,
                y=key_components[:, 1],
                name=cat,
                mode="markers",
                marker=dict(size=cat_counts),
            )
        )
